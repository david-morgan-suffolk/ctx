...
Update commands here when `pyproject.toml` scripts change. Update `.context/` when architecture, integrations, or durable decisions shift. Record warehouse, orchestrator, dataframe library, validation tool, and stage-naming choices in `.context/roadmap-notes.md` so they survive turnover.
